...
class HeapObject:
    ...
    def get_leak_size(self, period_dur=''):
        ...
        if self.samples:
            ...
            if len(result) < len(period_dur):
                raise NoTimeStepError("Ошибка при записи таймлайна: результат меньше заданных шагов")
        else:
...
def check_leak_with_timeline(result, leak_size_limit):
    """
