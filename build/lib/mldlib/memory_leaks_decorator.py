    ...
    def wrapper(obj):
        if inspect.isclass(obj):
            ...
            def test(*args, **kwargs):
                _wrapper_for_test(obj, timeline, host, port, ws, node_js,
...
def _wrapper_for_test(obj, timeline, host, port, ws, node_js, wait_func,
                      *args, **kwargs):
    """
    ...
    if conf.cdp:
        ...
        set_logger()
        cdp = obj.cdp = conf.cdp = DevToolsProtocolConnection(host=host,
                                                              port=port,
                                                              ws=ws)
        clear_conf_cdp = False
    ...
    if leak:
        ...
        conf.cdp = None if clear_conf_cdp else conf.cdp
...
def _meas_timeline(decorated_function, step_repeat, wait_func, node_js,
                   *args, **kwargs):
    """
    ...
    for i in range(step_repeat):
        ...
        if conf.default_wait_full_load and wait_func and not node_js:
            cdp.wait_full_load()
    ...
    return check_leak_with_timeline(steps=step_repeat, result=result,
